...
// Strict scratch experiment: keep the validated modal opening, then apply the
// current top-six exact-safety midgame with an explicit public capture
// probability gate/penalty. No api.rng(), seed recovery, board lookup tables,
...
function selectProbabilitySafetyMove(state, {
  ...
  const ranked = codex.rankedMoveScores(state, options)
    ...
    .filter(item => {
      ...
      return codex.captureProbability(from.strength, to.strength) >= minP;
    })
    .slice(0, topK);
  ...
  for (const item of ranked) {
    ...
    const score =
      item.score
      - Math.max(0, pPenaltyBelow - pCap) * pPenaltyWeight
...
function makeProbabilityGatedSafety({
  maxOpeningAttacks = 2,
  maxAttacks = 120,
  gapCut = 2,
  ...
  return function probabilityGatedSafety(api) {
    ...
    for (let attacks = 0; attacks < maxAttacks; attacks++) {
      ...
      const move = selectProbabilitySafetyMove(state, {
        ...moveOptions,
        threatenedWeight,
      });
      if (!move) return;
...
function candidates() {
  const out = [
    ...
    ['plain', () => makeProbabilityGatedSafety()],
  ];
  for (const minP of [0.2, 0.3, 0.4, 0.5, 0.55, 0.6]) {
    out.push([`minP${minP}`, () => makeProbabilityGatedSafety({ minP })]);
  }
  for (const pPenaltyBelow of [0.35, 0.45, 0.55]) {
    for (const pPenaltyWeight of [60, 120, 220]) {
      out.push([
        `pen${pPenaltyBelow}/${pPenaltyWeight}`,
        () => makeProbabilityGatedSafety({ pPenaltyBelow, pPenaltyWeight }),
      ]);
    }
  }
  for (const candidatePool of [6, 10, 16, 24]) {
    out.push([`pool${candidatePool}`, () => makeProbabilityGatedSafety({ candidatePool })]);
  }
  return out;
}
...
function main() {
  ...
  for (const row of rows.slice(0, 40)) {
    console.log(`${row.name.padEnd(18)} ${String(row.total).padStart(4)}/${row.totalGames}  ${row.parts.join('  ')}  ${row.msPerGame.toFixed(2)}ms/game`);
  }
}
...
module.exports = {
  makeProbabilityGatedSafety,
  selectProbabilitySafetyMove,
};
