...
// Strict scratch experiment: choose between two nearby gap-safety parameter
// sets from current public board features. No api.rng(), seed recovery, board
// lookup tables, live-node mutation, or benchmark-order state.
...
const CONFIGS = {
  ...
  top5: { topK: 5, countWeight: 12 },
  fast: { topK: 2, countWeight: 4 },
  cautious215: { topK: 6, countWeight: 14, minScore: 215 },
  cautious220: { topK: 6, countWeight: 14, minScore: 220 },
  cautious225: { topK: 6, countWeight: 14, minScore: 225 },
  ...
  large28: { topK: 6, countWeight: 14, largestWeight: 28 },
  ...
  rescueWide: { topK: 8, countWeight: 12, minScore: 200, redGainWeight: 36 },
};
...
function makeDynamicGapConfig({
  rule = null,
  matchConfig = CONFIGS.current,
  missConfig = CONFIGS.top5,
  ...
  return function dynamicGapConfig(api) {
...
function candidateRules(pair = 'current-top5', { targeted = false } = {}) {
  ...
  const out = [
    [aName, () => makeDynamicGapConfig({ rule: { feature: 'red', op: '>=', cut: -1 }, matchConfig: a, missConfig: a })],
    [bName, () => makeDynamicGapConfig({ rule: { feature: 'red', op: '>=', cut: -1 }, matchConfig: b, missConfig: b })],
  ];
...
function main() {
  ...
  const pair = argValue('pair') || 'current-top5';
  const targeted = process.argv.includes('--targeted');
  const rows = [];
  for (const [name, factory] of candidateRules(pair, { targeted })) {
