...
assert.equal(history.length, 39, 'games with missing replay data are silently omitted');
assert.equal(history.at(-1).ri, 38, 'retained scores preserve their actual game numbers');
assert.equal(compare(missing).state, 'better');
...
assert.equal(skillTrend(skillHistory(partial)).at(-1).trend, null);
assert.notEqual(skillTrend(history).at(-1).trend, null, 'unavailable games do not leave chart gaps');
