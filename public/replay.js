...
export function createReplayInspector(api) {
  const cache = [];
  return async (seed, moves, k) => {
    if (!Number.isInteger(k) || k < 0 || k >= moves.length) throw new Error('invalid move index');
    let entry = cache.find(e => e.seed === seed && e.moves === moves && e.length === moves.length);
    if (!entry) {
      entry = { seed, moves, length: moves.length };
      entry.result = api('/api/replay', 'POST', { seed, moves }).then(r => {
